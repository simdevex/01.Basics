'''
Python program which reads a text (only alphabetical
characters and spaces.) and prints two words. The first one is the
word which is arise most frequently in the text. The second one is
the word which has the maximum number of letters

Note: A word is a sequence of letters which is separated by the
spaces.
Input:
...
c. There is only one word which is arise most frequently in given
text.
